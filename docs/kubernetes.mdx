...
<WithVersions language="bash">
  { versions =>
    <CodeBlock className="language-bash">
      kubectl apply -f https://raw.githubusercontent.com/parca-dev/parca/{versions.server}/deploy/kubernetes-manifest.yaml
    </CodeBlock>
  }
</WithVersions>

You can verify by selecting pods if everthing runs as expected:
```shell
kubectl get pods -A
```
...
One of the cool features of Parca Agent is by default it doscovers all the containers run on the nodes that it's been deployed.
...
The relevant manifest cahanges on `parca-agent-daemonSet.yaml` would like the following:
```diff
...
template:
  metadata:
